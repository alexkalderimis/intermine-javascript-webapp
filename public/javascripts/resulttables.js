var query_path = "/query/results";
var query_to_list_path = "/query/tolist/json";
var list_path = "/lists/json";
var modelPath = "/model/json";
var code_path = "/query/code";
var fieldPath = "/summaryfields";
...
function loadBox(pq, base, id, token) {
    var params = {format: "jsoncount", query: pq.serialise(), token: token};
    var $tableContainer = $('#' + id).addClass("table-container");
    var dt_id = id + '_datatable';
    $tableContainer.append('<table id="' + dt_id + '"></table>');
...
var formatItemForNewState = function(path, op, item, pq, $dataTable, $conBox, clicked) {
    ...
    var $button = $con.find('button.delete-confirm');
    if ($button.length < 1) {
        $button = jQuery('<button class="delete-confirm btn constraint-deleter"><span class="ui-icon"></span></button>').appendTo($con);
    }
    $button.find('span').removeClass('ui-icon-circle-close').addClass('ui-icon-circle-check');
    $button.unbind('click').click(function() {
        var con = {path: path, op: op, values: $con.find('.value').map(function(i, e) { return $(e).text()}).get()};
...
function initTable(pq, id, base, token) {
    ...
    var setup_params = {format: "jsontable", query: pq.serialise(), token: token};
        ...
        $.ajax( {
            ...
            success: function(result) {
                var $dataTable = $('#' + id).dataTable( {
                    ...
                    "bRetrieve": true,
                    "sDom": '<"H"<"title"><"toolbar">RClr>t<"F"ip>',
                    "sScrollY": scrollY + "px",
                    "bServerSide": true,
                    "sAjaxSource": url,
                    ...
                    "fnDrawCallback": function() {
                        ...
                        __(pq.where).groupBy(function(con) {return con.path}).each(function(constraints, path) {
                            var title = result.columnHeaders[_(pq.select).indexOf(path)] || path;
                        ...
                        var ret = {
                            ...
                            "fnRender": function(obj) {
                                ...
                                if (typeof obj.aData[col] === "string") {
                                    ...
                                    ret += '<a href="' + base.replace(/\/?service\/?/, "") + obj.aData[col].url + '">';
                                    if (obj.aData[col].value == null) {
                                        ret += 'no value</a></span>';
                                    } else {
                ...
                var $buttonset = jQuery('<div class=btn-group>');
                var $coder = jQuery('<a class=btn href=#>').text("API");
                var $codeSelector = jQuery('<a class="btn dropdown-toggle" data-toggle="dropdown">').append('<span class=caret></span>');
                var $codeOptions = $('<ul class=dropdown-menu>');
                $buttonset.append($coder).append($codeSelector).append($codeOptions);
                ...
                var getCodeShower = function(lang) { return function() {
                    ...
                    $('body .lang-menu').remove();
                    var visibleQuery = getVisibleQuery(pq, $dataTable, id);
                    $.get(base + code_path, {token: tokens[id], query: visibleQuery.serialise(), lang: lang}, function(res) {
                        var $popup = jQuery('<div class="summary-popup lang-popup"></div>');
                ...
                _(langs).each(function(l) {
                    var $li = $('<li><a href=#>' + l + '</a></li>');
                    if (l === "XML") {
                        $li.click(function() {
                            showXML(); 
                            $coder.unbind('click').click(showXML);
                        });
                    } else if ( l === "URL" ) {
                        $li.click(function() {
                            showURL(); 
                            $coder.unbind('click').click(showURL);
                        });
                    } else {
                        var shower = getCodeShower(l);
                        $li.click(function() {
                            shower();
                            $coder.unbind('click').click(shower);
                        });
                    }
                    $li.appendTo($codeOptions);
                });
                ...
                if (InterMine.settings.defaultAPIFormat) {
                    ...
                    coderCallBack = _($codeSelector.click).bind($codeSelector);
                }
                ...
                var $dls = jQuery('<div class=btn-group>');
                var $downloader = jQuery('<a class=btn href=#>').text("Download");
                var $dlSelector = jQuery('<a class="btn dropdown-toggle" href=#>').
                                    append('<span class=caret></span>');
                var $dlmenu = jQuery('<ul class=dropdown-menu>');
                $dls.append($downloader).append($dlSelector).append($dlmenu);
                $dt.find('div.toolbar').append($dls);

                var formats = ["TSV", "CSV", "XML", "JSONROWS"];
                _(formats).each(function(f) {
                    var li = $('<li><a href=#>' + f + '</a></li>');
                    var downloader = getDownloader(f);
                    li.click(function() {
                        downloader(); 
                        $downloader.unbind("click").click(downloader)
                    });
                });
                ...
                if (InterMine.settings.defaultExportFormat) {
                    ...
                    downloaderCallBack = function() {$dlSelector.click()};
                }
                $downloader.click(downloaderCallBack);

                var $listButton = jQuery('<a class=btn href=#>')
                    .text("Create List")
                    .append('<i class=icon-list></i>')
                    .click(function() {
                    chosenListIds[id] = [];
                    typeOfObj[id] = {};
                ...
                $button
                    ...
                    .click(function() {
                        ...
                        $datatable.parent().find('.constraints-hider').slideUp();
                ...
                */
                ...
                $originalDiv = $dt.parent();
                $('<div class="constraints-hider"><h3 style="display: inline; cursor: pointer;"><span class="constraint-count"></span> Constraint<span class="plural-s">s</span></h3><div id="' + id + '_constraints" style="display:none;">Loading...</div></div>').prependTo($originalDiv);
